...
def parse_pages(pages, config: ParserConfig = None) -> list:
    ...
    are only a backup.
    """
    config = ParserConfig() if not config else config
...
def unite_hurenkind(pages: list) -> list:
    ...
    for page in pages[1:]:
        ...
        if not pagebefore_exists:
            result.append(page)
            continue
        befored = before(page, right)
        if befored:
            # move hurenkinds to page before
            result[-1][-1].extend(befored)
            # remove hurenkind from current page
            result.append(page[len(befored):])
            continue
        result.append(page)
    return result
